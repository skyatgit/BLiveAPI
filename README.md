...
[TargetCmd("SEND_GIFT")]
private void OnSendGiftEvent(object sender, (string cmd, string hitCmd, JObject jsonRawData, byte[] rawData) e)
{
}
...
[TargetCmd("ALL")]
private void OnAllEvent(object sender, (string cmd, string hitCmd, JObject jsonRawData, byte[] rawData) e)
{
}
//当携带OTHERS时,该方法会接收未被其他方法命中的SMS消息,但TargetCmd参数列表中的其他命令被命中时不会被再次Invoke
[TargetCmd("OTHERS")]
private  void OtherMessagesEvent(object sender, (string cmd, string hitCmd, JObject jsonRawData, byte[] rawData) e)
{
}
